...
if 'vote_average' in movies.columns and 'rating' not in movies.columns:
    ...
    try:
        if isinstance(x, str) and x.strip().startswith('['):
            ...
            if isinstance(data, list):
             ...
             # Fallback for space-separated or messy strings
             # Remove [ ] ' " just in case
             ...
             if isinstance(x, list):
                 return [item.get(key) for item in x if isinstance(item, dict)]
    except:
        pass
    return []
...
plt.xlabel('Budget ($)')
plt.ylabel('Revenue ($)')
plt.savefig('budget_revenue_scatter.png')
plt.show()
...
plt.savefig('genre_ratings_boxplot.png')
plt.show()
...
scale_cols = ['budget', 'revenue', 'runtime', 'vote_count', 'rating', 'roi']
...
groups = [movies[movies['primary_genre'] == g]['rating'] for g in top_5_genres if not movies[movies['primary_genre'] == g].empty]
if len(groups) > 1:
    f_stat, p_val4 = stats.f_oneway(*groups)
else:
    f_stat, p_val4 = np.nan, np.nan
...
pca_features = ['budget', 'revenue', 'rating', 'runtime', 'vote_count', 'popularity']
# Ensure all exist
